...
#pragma omp task depend(in:A[k][i]) firstprivate(k, i, dst)
{
            ...
            if (block_rank[k*nt+i] != mype) {
                ...
                if (recv_flag) {
#pragma omp task depend(out:C[i]) firstprivate(k, i) shared(C, block_rank)
{
                    MPI_Request recv_req;
...
#pragma omp task depend(in:C[i], A[k][j]) depend(out:A[j][i]) firstprivate(k, j, i) shared(C, A)
                        gemm(C[i], A[k][j], A[j][i], ts, ts);
                    } else if (block_rank[k*nt+i] == mype && block_rank[k*nt+j] != mype) {
#pragma omp task depend(in:A[k][i], C[j]) depend(out:A[j][i]) firstprivate(k, j, i) shared(C, A)
                        gemm(A[k][i], C[j], A[j][i], ts, ts);
                    } else {
#pragma omp task depend(in:C[i], C[j]) depend(out:A[j][i]) firstprivate(k, j, i) shared(C, A)
...
#pragma omp task depend(in:C[i]) depend(out:A[i][i]) firstprivate(k, i) shared(C, A)
