...
using size_spec_t  = dash::SizeSpec<3>;
using dist_spec_t  = dash::DistributionSpec<3>;
using team_spec_t  = dash::TeamSpec<3>;
  ...
  for (auto l = 0; l < level; ++l) {
    for (auto r = 0; r < rows; ++r) {
      ...
      for (auto c = 0; c < cols; ++c)
        cout << " " << setw(3) << (double)(*matrix)[l][r][c];
      cout << endl;
    }
    cout << "-----" << endl;
  }
...
int main(int argc, char *argv[])
{
  ...
  dist_spec_t dist(dash::BLOCKED, dash::BLOCKED, dash::BLOCKED);
  team_spec_t tspec(ranks,1,1);

  tspec.balance_extents();

  pattern_t pattern(size_spec_t(matrix_ext, matrix_ext, matrix_ext), dist, tspec,
                    dash::Team::All());
...
//#endif
  ...
  CycleSpecT cycle{Cycle::FIXED, Cycle::FIXED, Cycle::FIXED};
  ...
  */
  ...
  auto fixed_func = [&matrix_ext](const std::array<dash::default_index_t,3>& coords) {
      auto boundary = matrix_ext - 1;

      // everything which is not on the bottom in z-direction gets 0.0
      /*
      if ( 0 < coords[0]) return 0.0;

      // if on the bottom in z-direction then one quarter gets 1.0
      if ( ( coords[1] < boundary/2 ) && ( coords[2] < boundary/2 ) ) {
  ...
  for (auto d = 0; d < iterations; ++d) {
    ...
    for(auto it = current_halo->ibegin(); it != it_end; ++it)
    {
      auto core = *it;
      auto dtheta =
            (it.valueAt(0) + it.valueAt(1) - 2 * core) / (dz * dz) +
            (it.valueAt(2) + it.valueAt(3) - 2 * core) / (dy * dy) +
            (it.valueAt(4) + it.valueAt(5) - 2 * core) / (dx * dx);
