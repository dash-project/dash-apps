...
void make_local_matrix(HPC_Sparse_Matrix * A)
{
  ...
  *((int*)A->signal_win.baseptr) = 0;
  ...
  // post recv
  A->gptr.resize(num_recv_neighbors, DART_GPTR_NULL);
  for(int i=0; i<num_recv_neighbors; ++i) {
    MPI_Irecv(&A->gptr[i], sizeof(dart_gptr_t), MPI_BYTE, neighbors[i],
              MPI_MY_TAG, MPI_COMM_WORLD, &request[i]);
  }

  for(int i=0; i<num_recv_neighbors; ++i) {
    MPI_Send(&gptr, sizeof(dart_gptr_t), MPI_BYTE, recv_list[i], MPI_MY_TAG, MPI_COMM_WORLD);
    dart_gptr_incaddr(&gptr, recv_length[i]*sizeof(double));
  }
