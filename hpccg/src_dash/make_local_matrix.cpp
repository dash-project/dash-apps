...
void make_local_matrix(HPC_Sparse_Matrix * A)
{
  ...
  int total_recv_length = j;
  A->total_received_elements = j;
  ...
  ////////////////////////////////////////////////////////////////////////
  // Create windows for one-sided communication
  ////////////////////////////////////////////////////////////////////////
  ...
  dash::SizeSpec<2> sizespec(dash::size(), max_recv_length);
  dash::DistributionSpec<2> distspec(dash::BLOCKED, dash::NONE);
  dash::TeamSpec<2> teamspec(dash::size(), 1);

  A->data.allocate(sizespec, distspec, teamspec);
  ...
  // post recv
  A->offset.resize(num_recv_neighbors, 0);
  for(int i=0; i<num_recv_neighbors; ++i) {
    MPI_Irecv(&A->offset[i], 1, MPI_INT, neighbors[i],
              MPI_MY_TAG, MPI_COMM_WORLD, &request[i]);
  }

  int loffset = 0;
  for(int i=0; i<num_recv_neighbors; ++i) {
    MPI_Send(&loffset, 1, MPI_INT, recv_list[i], MPI_MY_TAG, MPI_COMM_WORLD);
    loffset += recv_length[i];
  }

  // receive the displacement from each neighbor
...
//
//  // Allocate the signaling window
//  MPI_Win_allocate(sizeof(int), 1, MPI_INFO_NULL, MPI_COMM_WORLD,
//                   &A->signal_win.baseptr, &A->signal_win.win);
//
//  MPI_Win_lock_all(0, A->signal_win.win);
//
...
//  // Allocate the data window
//  MPI_Win_allocate(
//    sizeof(double)*total_recv_length, sizeof(double),
//    MPI_INFO_NULL, MPI_COMM_WORLD,
//    &A->data_win.baseptr, &A->data_win.win);
//  A->data_win.size = sizeof(double)*total_recv_length;
//  MPI_Win_lock_all(0, A->data_win.win);
//
//  ////////////////////////////////////////////////////////////////////////
//  // Exchange displacements
//  ////////////////////////////////////////////////////////////////////////
...
//
//  // post recv
//  A->disps.resize(num_recv_neighbors, 0);
//  for(int i=0; i<num_recv_neighbors; ++i) {
//    MPI_Irecv(&A->disps[i], 1, MPI_AINT, neighbors[i],
//              MPI_MY_TAG, MPI_COMM_WORLD, &request[i]);
//  }
//
//  MPI_Aint disp = 0;
////  std::cout << "["<< myproc << "] " << total_recv_length << std::endl;
//  for(int i=0; i<num_recv_neighbors; ++i) {
////    std::cout << "["<< myproc << "] " << recv_length[i] << " from " << recv_list[i] << " at disp " << disp << std::endl;
//    MPI_Send(&disp, 1, MPI_AINT, recv_list[i], MPI_MY_TAG, MPI_COMM_WORLD);
//    disp += recv_length[i];
//  }
//
//  // receive the displacement from each neighbor
