...
int main( int argc, char* argv[] ) {
    ...
    if ( 0 == myid ) {
        ...
        for ( uint32_t strip = 0; strip < numstrips; strip++ ) {
            ...
            /* do not fix RGB byte here order but do it in parallel below */
    ...
    the RGB byte order, fix it by swaping red and blue.
    Then again, we don't care about the colors for the rest of the code,
    and we can leave this out entirely. */
    {
        std::chrono::time_point<std::chrono::system_clock> start, end;
        start = std::chrono::system_clock::now();

        /* std::for_each with _local_ iterators works but
        dash::for_each with global iterator won't work because
        it expects only const accesses to the elements! */

        //dash::for_each( matrix.begin(), matrix.end(), [](RGB& rgb){
        std::for_each( matrix.lbegin(), matrix.lend(), [](RGB& rgb){
              std::swap<uint8_t>( rgb.r, rgb.b );
        } );

        end = std::chrono::system_clock::now();
        cout << "    swapped rgb in parallel in "<< std::chrono::duration_cast<std::chrono::seconds> (end-start).count() << " seconds" << endl;
    }
