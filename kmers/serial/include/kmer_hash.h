...
int add_kmer(hash_table_t *hashtable, memory_heap_t *memory_heap, const unsigned char *kmer, char left_ext, char right_ext, int count)
{
   ...
   if (count < 5) {
     char sub[LINE_SIZE];
     strncpy(sub, (char *) kmer, LINE_SIZE);
     printf("original: %.*s, packed: %.*s, packed_length: %d\n",  LINE_SIZE - 1, sub, KMER_PACKED_LENGTH, packedKmer, KMER_PACKED_LENGTH);
   }
   int64_t hashval = hashkmer(hashtable->size, (char*) packedKmer);
