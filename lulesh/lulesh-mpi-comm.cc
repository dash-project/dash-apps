// ==============================
// == DASH port of LULESH 2.03 ==
// ==============================
//
// These are the five raw MPI communication routines from LULESH 2.03
//
//  - void CommRecv()
//  - void CommSend()
//  - void CommSBN()
...
void CommRecv(Domain& domain, MPIComm& comm, int msgType, Index_t xferFields,
   ...
   for (Index_t i=0; i<26; ++i) {
     comm.recvRequest[i] = MPI_REQUEST_NULL ;
   }
   ...
   if (planeMin && doRecv) {
      ...
      MPI_Irecv(&comm.commDataRecv[pmsg * maxPlaneComm],
                recvCount, baseType, fromRank, msgType,
                MPI_COMM_WORLD, &comm.recvRequest[pmsg]) ;
      ++pmsg ;
   }
   if (planeMax) {
      /* contiguous memory */
   ...
   if (!planeOnly) {
      ...
      if (rowMin && colMin && doRecv) {
         ...
         MPI_Irecv(&comm.commDataRecv[pmsg * maxPlaneComm +
                   ...
                   MPI_COMM_WORLD, &comm.recvRequest[pmsg+emsg]) ;
         ++emsg ;
      }
      ...
      if (rowMin && colMin && planeMin && doRecv) {
         ...
         MPI_Irecv(&comm.commDataRecv[pmsg * maxPlaneComm +
                   ...
                   MPI_COMM_WORLD, &comm.recvRequest[pmsg+emsg+cmsg]) ;
         ++cmsg ;
      }
              ...
              Index_t dx, Index_t dy, Index_t dz, bool doSend, bool planeOnly)
{
   ...
   for (Index_t i=0; i<26; ++i) {
      comm.sendRequest[i] = MPI_REQUEST_NULL ;
   }
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
         ...
         MPI_Isend(destAddr, xferFields*sendCount, baseType,
                   ...
                   MPI_COMM_WORLD, &comm.sendRequest[pmsg]) ;
         ++pmsg ;
      }
      if (planeMax && doSend) {
         destAddr = &comm.commDataSend[pmsg * maxPlaneComm] ;
   ...
   if (!planeOnly) {
      if (rowMin && colMin) {
         ...
         MPI_Isend(destAddr, xferFields*dz, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &comm.sendRequest[pmsg+emsg]) ;
         ++emsg ;
      }
      ...
      if (rowMin && colMin && planeMin) {
         ...
         MPI_Isend(comBuf, xferFields, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &comm.sendRequest[pmsg+emsg+cmsg]) ;
         ++cmsg ;
      }
   ...
   MPI_Waitall(26, comm.sendRequest, status) ;
}
...
void CommSBN(Domain& domain, MPIComm& comm,
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
         /* contiguous memory */
         srcAddr = &comm.commDataRecv[pmsg * maxPlaneComm] ;
   ...
   if (rowMin & colMin & planeMin) {
      /* corner at domain logical coord (0, 0, 0) */
      Real_t *comBuf = &comm.commDataRecv[pmsg * maxPlaneComm +
                                             emsg * maxEdgeComm +
                                      cmsg * CACHE_COHERENCE_PAD_REAL] ;
