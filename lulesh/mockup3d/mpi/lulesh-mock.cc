...
int main(int argc, char* argv[])
{
  ...
  // 4x3x2 local elements and 2x2x2 process arrangement
  Domain dom({{4,3,2}}, {{2,2,2}});

  if( dom.numProcs()!=dom.px()*dom.py()*dom.pz() ) {
    if( dom.myRank()==0 ) {
      std::cerr << "Err: this program must be run with " <<
	dom.px()*dom.py()*dom.pz() << " processes!" << std::endl;
    }
    MPI_Finalize();
    return 1;
  }
