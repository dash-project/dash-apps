...
packed_edge * create_graph_tuples() {
  ...
  generate_kronecker_range(seed, scale, start_edge, start_edge
      + local_edge_count, edges);
  return edges;
}
 ...
 */
std::vector<dash::GraphNode> & get_assigned_nodes(std::vector<dash::GraphNode> 
    & neighbours const, std::vector<dash::GraphNode> & prefix const) {
 ...
 */
void add_to_visited(std::vector<dash::GraphNode> & visited,
    std::vector<dash::GraphNode> & neighbours const) {
  // TODO: to be implemented
}
...
int main() {
  ...
  // dash::1DGraphPartitioning: each processor holds the same amount of
  // vertices
  // dash::2DGraphPartitioning: each processor holds roughly the same amount
  // of edges
  ...
  auto partition_pos = partition.get_position(myid);
  int partition_row = partition_pos.row;
  int partition_row_index = partition_pos.column;
  int partition_columns = partition.get_column_count();
  dart_unit_t next_unit = partition.get_unit(partition_row, 
      partition_column + 1);;
  dart_unit_t prev_unit = partition.get_unit(partition_row,
      partition_column - 1);
  dart_unit_t transposed_unit = partition.get_unit(partition_row_index,
      partition_row); //(i, j) -> (j, i)
  dart_unit_t root = partition.get_unit(partition_row, partition_columns - 1);
  dart_team_t row_team = partition.team_in_row(partition_row);
    ...
    while(1) {
      if(!frontier.empty()) {
        ...
        for(auto el : frontier) {
          ...
          std::vector<dash::GraphNode> current = graph.get_lneighbours(el);
        ...
        if(partition_row_index < partition_columns - 1) {
          if(partition_row_index > 0) {
            ...
            forward.insert(forward.end(), neighbours.begin(),
                neighbours.end());
            dart_send(static_cast<void *>(forward), forward.size()
                * sizeof(dash::GraphNode), next_unit);
            delete forward;
          } else {
            // send prefix to next processor in partition row
            dart_send(static_cast<void *>(neighbours), neighbours.size()
                * sizeof(dash::GraphNode), next_unit);
          }
        ...
        dart_bcast(static_cast<void *>(neighbours), neighbours.size()
            * sizeof(dash::GraphNode), root, row_team);
        ...
        std::vector<dash::GraphNode> trans_neighbours;
        dart_sendrecv(static_cast<void *>(neighbours), neighbours.size()
            * sizeof(dash::GraphNode), , transposed_unit, 0, 
            static_cast<void *>(trans_neighbours), , , transposed_unit, 0);
