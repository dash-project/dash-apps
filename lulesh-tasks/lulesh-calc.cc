...
//#pragma omp parallel for firstprivate(numElem, deltaTime)
  dash::tasks::taskloop(Index_t{0}, numElem, numElem/(dash::tasks::numthreads()*DASH_TASKLOOP_FACTOR),
...
/* ================================================================
   Caller/callee relationship for CalcTimeConstraintsForElems()
...
{
  struct per_thread_elem {
    Real_t  dtcourant;
    Index_t courant_elem;
  };
  dash::tasks::Combinator<per_thread_elem> reduction({{dtcourant, -1}});
...
//#pragma omp parallel firstprivate(length, qqc)
  dash::tasks::taskloop(Index_t{0}, length, length/(dash::tasks::numthreads()*DASH_TASKLOOP_FACTOR),
    [&](Index_t from, Index_t to){
      ...
      Index_t&  courant_elem  = reduction.local().courant_elem;
...
//  #pragma omp for
  ...
  auto d = reduction.reduce([](const per_thread_elem& a,
                               const per_thread_elem& b){
    per_thread_elem res = b;
    if (a.dtcourant < b.dtcourant ) {
      res = a;
    }
    return res;
  });
				 ...
				 Real_t dvovmax, Real_t& dthydro)
{
  ...
  dash::tasks::Combinator<per_thread_elem> reduction({{dthydro, -1}});
...
//#pragma omp parallel firstprivate(length, dvovmax)
  ...
  dash::tasks::taskloop(Index_t{0}, length, length/(dash::tasks::numthreads()*DASH_TASKLOOP_FACTOR),
    [&](Index_t from, Index_t to) {
      ...
      Index_t& hydro_elem  = reduction.local().hydro_elem;
...
/* ================================================================
   Caller/callee relationship for CalcVolumeForceForElems()

   CalcVolumeForceForElems()
    |-- 4x Allocate<Real_t>(numElem)
    ...
    |    |-- 6x Allocate<Real_t>(8*numElem)
    |    |-- CollectDomainNodesToElemNodes()
    |    |-- CalcElemVolumeDerivative()
    |    |    +-- VoluDer()
    |    |-- CalcFBHourglassForceForElems()
    |    |    |-- 3x Allocate<Real_t>(8*numElem)
...
//#pragma omp parallel for firstprivate(numNode)
    dash::tasks::taskloop(Index_t{0}, numNode, numNode/(dash::tasks::numthreads()*DASH_TASKLOOP_FACTOR),
      [&](Index_t from, Index_t to) {
...
void CalcEnergyForElems(Real_t* p_new, Real_t* e_new, Real_t* q_new,
                        ...
                        Index_t length, Index_t *regElemList)
{
  // TODO: we need to encapsulate allocation of pHalfStep into a task
...
//#pragma omp parallel for firstprivate(rho0, ss4o3)
  dash::tasks::taskloop(Index_t{0}, len, len/(dash::tasks::numthreads()*DASH_TASKLOOP_FACTOR),
    ...
    (Index_t from, Index_t to, dash::tasks::DependencyVectorInserter deps){
      ...
      *deps = dash::tasks::in(&vnewc[from]);
      *deps = dash::tasks::in(&domain.ss(from));
    }*/);
  //dash::tasks::complete();
}
