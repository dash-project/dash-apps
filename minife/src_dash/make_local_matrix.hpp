...
make_local_matrix(MatrixType& A)
{
  ...
  for(int i=0; i<num_recv_neighbors; ++i) {
    MPI_Irecv(&A.offset[i], 1, TypeTraits<LocalOrdinal>::mpi_type(), neighbors[i],
              MPI_MY_TAG, MPI_COMM_WORLD, &request[i]);
  }

  LocalOrdinal loffset = 0;
  for(int i=0; i<num_recv_neighbors; ++i) {
    MPI_Send(&loffset, 1, TypeTraits<LocalOrdinal>::mpi_type(), recv_list[i], MPI_MY_TAG, MPI_COMM_WORLD);
