...
//  std::cout << "entering begin_exchange_externals\n";
  ...
  for(int i=0; i<num_neighbors; ++i) {
    ...
    MPI_Put(s_buffer, n_send, mpi_dtype, neighbors[i], A.disps[i], n_send,
      mpi_dtype, A.data_win.win);
    s_buffer += n_send;
  }
#endif
}
...
finish_exchange_externals(MatrixType& A, VectorType& x)
{
  ...
  MPI_Win_flush_all(A.data_win.win);

  int val = 1;
  const std::vector<int>& neighbors = A.neighbors;
  // signal completion
  for(int i=0; i<num_neighbors; ++i) {
    MPI_Accumulate(&val, 1, MPI_INT, neighbors[i], 0, 1,
        MPI_INT, MPI_SUM, A.signal_win.win);
  }
  ...
  do {
    ...
    MPI_Compare_and_swap(
      &zero, &num_neighbors, &flag, MPI_INT, myproc, 0, A.signal_win.win);
