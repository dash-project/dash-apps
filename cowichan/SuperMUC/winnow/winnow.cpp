...
inline void ReadMatricesAndNelts( NArray<T,2>& randMat, NArray<bool,2>& threshMask )
{
  ...
  if(0 == myid)
  {
    //not used in SuperMUC benchmark
    // if (!is_bench) {
      // //read matrices
      // int tmp;
...
inline void FillOnTheFly( NArray<T,2>& randMat, NArray<bool,2>& threshMask )
{
  ...
  {
    *ptr = (static_cast<size_t>(i)*in.ncols+j) % 1000;
    if(++j == in.ncols) {++i;j=0;}
  }
  ...
  {
    if(( ( static_cast<size_t>(i)*in.ncols+j) % threshInverse ) == 0) {
      *ptr = true;
    }else{
      *ptr = false;
    }
